...
from timeseriesflattener.v1.misc_utils import df_contains_duplicates


class ValidateInitFlattenedDataset:
    """Validator for a flattened dataset."""

    def __init__(self, df: pd.DataFrame, timestamp_col_name: str, entity_id_col_name: str):
        self.df = df
    ...
    def _check_for_duplicate_rows(self):
        ...
        if df_contains_duplicates(
            df=self.df, col_subset=[self.entity_id_col_name, self.timestamp_col_name]
        ):
            raise ValueError("Duplicate id/timestamp combinations in prediction_times_df, aborting")
