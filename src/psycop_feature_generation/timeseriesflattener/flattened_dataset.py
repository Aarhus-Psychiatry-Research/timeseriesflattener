...
class FlattenedDataset:  # pylint: disable=too-many-instance-attributes
    ...
    def add_static_info(
        ...
        if output_col_name_override is None:
            output_col_name_override = f"{prefix}_{value_col_name}"
        else:
            output_col_name_override = f"{prefix}_{output_col_name_override}"

        df = pd.DataFrame(
            {
                self.id_col_name: spec.values_df[self.id_col_name],
                output_col_name_override: spec.values_df[value_col_name],
            },
        )
