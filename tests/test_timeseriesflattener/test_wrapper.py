...
def test_output_independent_of_order_of_input():
    ...
    flattened_dataset1.add_temporal_predictors_from_pred_specs(
        predictor_specs=predictor_list,
        preloaded_predictor_dfs={"predictor_df": predictor_df},
    )

    flattened_dataset2.add_temporal_predictors_from_pred_specs(
        predictor_specs=predictor_list2,
        preloaded_predictor_dfs={"predictor_df": predictor_df},
    )
...
def test_wrong_formatting():
    ...
    with pytest.raises(ValueError):
        ...
        flattened_dataset.add_temporal_predictors_from_pred_specs(
            predictor_specs=unresolvable_df,
        )
    ...
    with pytest.raises(ValueError):
        ...
        flattened_dataset.add_temporal_predictors_from_pred_specs(
            predictor_specs=unresolvable_resolve_multiple,
            preloaded_predictor_dfs={"predictor_df": predictor_df},
        )
    ...
    with pytest.raises(ValueError):
        ...
        flattened_dataset.add_temporal_predictors_from_pred_specs(
            predictor_specs=lookbehind_days_not_int,
            preloaded_predictor_dfs={"predictor_df": predictor_df},
        )
